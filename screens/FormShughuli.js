...
const FormShughuli = ({ route }) => {
  ...
  let jumla_ = 0;
  const [mahudhurioJumla, setMahudhurioJumla] = useState(mahudhurio_jumla);
  const [mahudhurioWaume, setMahudhurioWaume] = useState(waume);
  const [mahudhurioWake, setMahudhurioWake] = useState(wake);
  const [idadiHisa, setIdadiHisa] = useState(idadi_hisa);
  const [adhabu, setAdhabu] = useState(0);
  const [afya, setAfya] = useState(afya_);
  const [elimu, setElimu] = useState(elimu_);
  ...
  const [bimaMkopo, setBimaMkopo] = useState(bima_);
  const [mapatoMengine, setMapatoMengine] = useState(0);
  const [jumla, setJumla] = useState(jumla_);
  const [jumlaKuu, setJumlaKuu] = useState(jumla_);
      ...
      <KeyboardAwareScrollView>
        ...
        >
          ...
          <CustomInput
            ...
            isNumber={true}
          />
          {states.formsFilled.includes("shughuli") && (
            <View
              style={{
                width: "100%",
